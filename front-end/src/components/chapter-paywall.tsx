...
export function ChapterPaywall({chapter, whitelist}: Props){
    ...
    return (
        <div className="mx-auto w-1/2 max-w-2xl flex-shrink-0 flex flex-col gap-4 justify-center items-center lg:max-w-xl">
            <div className="flex justify-center items-center gap-4 w-full">
                {
                    ! hasApplied && (
                        <>
                            <ApplyToGiveFeedback chapter={chapter} className="w-1/2 border-2 rounded-none">
                                <MessageCircleMore className="w-5 h-5 flex-none" />
                                Apply to Give Feedback
                            </ApplyToGiveFeedback>
                            <span className="text-xs font-mono text-primary/50 uppercase">or</span>
                        </>
                    )
                }
            ...
            <span className="text-xs mx-auto font-mono text-primary/50 uppercase">to read the chapter</span>
        </div>
    )
}
